...
if (bool_moving and not bool_special) {
	...
	int_target_x = obj_pacman.x;
	int_target_y = obj_pacman.y;
	
	int_min = 9999;
	
	if (bool_right) {
		int_distance = point_distance(x + sprite_width, y, int_target_x, int_target_y);
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 0;
			sprite_index = spr_ghost_red_right;
		}
	}
	
	if (bool_up) {
		int_distance = point_distance(x, y - sprite_height, int_target_x, int_target_y);
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 90;
			sprite_index = spr_ghost_red_up;
		}
	}
	...
	if (bool_left) {
		...
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 180;
			sprite_index = spr_ghost_red_left;
		}
	}
	...
	if (bool_down) {
		...
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 270;
			sprite_index = spr_ghost_red_down;
		}
	}
...
if (bool_special and bool_eaten) {
	...
	if (bool_right) {
		int_distance = point_distance(x + sprite_width, y, int_x_in, int_y_out_red);
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 0;
		}
	...
	if (bool_up) {
		int_distance = point_distance(x, y - sprite_height, int_x_in, int_y_out_red);
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 90;
		}
	...
	if (x == int_x_in and y == int_y_out_red) {
		bool_special = false;
		speed = 0;
	} else {
		speed = int_speed;
