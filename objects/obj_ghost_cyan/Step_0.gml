...
if (bool_special and bool_eaten) {
	...
	if (bool_right) {
		int_distance = point_distance(x + sprite_width, y, int_x_in, int_y_out_cyan);
	...
	if (bool_up) {
		int_distance = point_distance(x, y - sprite_height, int_x_in, int_y_out_cyan);
		if (int_distance < int_min) {
			int_min = int_distance;
			direction = 90;
		}
	...
	if (x == int_x_in and y == int_y_out_cyan) {
		bool_special = false;
		speed = 0;
	} else {
		speed = int_speed;
...
if (not bool_special and bool_eaten) {
	...
	if (y == int_y_in) {
		...
		bool_eaten = false;
		direction = 0;
		sprite_index = spr_ghost_cyan_right;
	}
}
